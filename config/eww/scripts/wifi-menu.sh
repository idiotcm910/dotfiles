...
if [[ "$(nmcli -t -f WIFI general)" != enabled ]]; then
  [[ "$(printf 'Bật Wi-Fi\n' | menu)" == 'Bật Wi-Fi' ]] && nmcli radio wifi on
  exit 0
fi
networks="$(nmcli -t --escape no -f IN-USE,SSID,SIGNAL,SECURITY device wifi list --rescan auto | awk -F: '$2 != "" && !seen[$2]++ { printf "%s  %s  %s%%  %s\\n", ($1 == "*" ? "●" : "○"), $2, $3, $4 }')"
...
case "$choice" in
  ...
  *)
    ...
    if nmcli -t -f NAME connection show | grep -Fxq "$ssid"; then nmcli connection up id "$ssid"
    else
