...
Bamboo, installer chính thức cho Codex CLI và npm user-scope cho Claude Code.
...
bash restore.sh --only ai        # Codex, Claude và portable config
bash restore.sh --skip-aur       # bỏ qua Google Chrome
```

Sau khi restore desktop, chạy `startx` để vào Qtile. Codex, Claude và Chrome
vẫn yêu cầu đăng nhập thủ công; credential và session không được lưu trong repo.
