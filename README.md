...
Bamboo, installer chính thức cho Codex CLI, npm cho Claude Code và Pi.
...
bash restore.sh --only ai        # Codex, Claude, Pi và portable config
bash restore.sh --skip-aur       # bỏ qua Google Chrome
```

Sau khi restore desktop, chạy `startx` để vào Qtile. Codex, Claude, Pi và
Chrome vẫn yêu cầu đăng nhập thủ công; credential và session không được lưu
trong repo.
